...
def _get_data_root() -> Path:
    """
    缓存根目录：与 Settings.DATA_ROOT（.env / 环境变量）同步；
    未配置时为相对本文件定位的 ``<backend>/data``（本文件在 app/libs/data_source/，上溯四级为 backend）。
    """
    from app.config import settings

    if settings.DATA_ROOT and str(settings.DATA_ROOT).strip():
...
def get_ticks(
    ...
    *,
    ...
    返回 list of dict（每行一条），列与迅投 get_market_data_ex period=tick 一致。
    """
